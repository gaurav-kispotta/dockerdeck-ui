...
function loadCompose(file: string) {
    const content = readFileSync(path.resolve(__dirname, '..','samples', file), 'utf-8');
...
describe('DockerComposeAstBuilder', () => {
    it('builds AST for docker-compose-9.yaml with expected services, networks and volumes', () => {
        ...
        expect(influx?.ports).toContainEqual({ internal: 8087, external: 8086 });
        expect(influx?.volumes.some(v => v.internal.includes('influxdb_data') || v.external.includes('influxdb_data'))).toBe(true);
